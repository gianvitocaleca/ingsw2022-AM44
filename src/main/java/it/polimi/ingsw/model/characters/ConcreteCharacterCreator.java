...
public class ConcreteCharacterCreator implements CharacterCreator {
    ...
    public Character createCharacter(Name name, Playable model) {
        if (name != null) {
            if (name.equals(Name.HERALD)){
                return new Herald(name, model);
                ...
                return new Thief(name, model);
                    ...
                    name.equals(Name.FARMER)){
                return new BehaviorCharacter(name, model);
            } else if (name.equals(Name.JOKER)){
                return new MoverCharacter(name, model,JOKER_CAPACITY);
            } else if (name.equals(Name.MINSTREL)){
                return new MoverCharacter(name, model,MINSTREL_CAPACITY);
            } else if (name.equals(Name.MONK)||name.equals(Name.PRINCESS)){
                return new MoverCharacter(name, model,PRINCESS_MONK_CAPACITY);
            }
        }
        return null;
    }
